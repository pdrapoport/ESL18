...
for i in range(0, len(x)):
    sp.append(x[i][0])
    sq.append(x[i][1])
    phi_kalman.append(x[i][2])
    theta_kalman.append(x[i][3])
    sr.append(x[i][4])
    sr_filtered.append(x[i][5])
    sax.append(x[i][6])
    say.append(x[i][7])
    sax_filtered.append(x[i][8])
    say_filtered.append(x[i][9])
    t.append(x[i][11])

sr_ref = butter_lowpass_filter(sr_filtered, cutoff, fs, order)
sax_ref = butter_lowpass_filter(sax_filtered, cutoff, fs, order)
say_ref = butter_lowpass_filter(say_filtered, cutoff, fs, order)

plt.figure()
plt.subplot(3,1,1)
plt.plot(t, sax_filtered,linewidth=0.75, label='sax_filtered')
plt.plot(t, sax_ref, linewidth=0.75, label='sax_ref')
plt.legend()

plt.subplot(3,1,2)
plt.plot(t, say_filtered,linewidth=0.75, label='say_filtered')
plt.plot(t, say_ref, linewidth=0.75, label='say_ref')
plt.legend()
...
#
# plt.figure()
# plt.plot(t,sax,t,sax_filtered,linewidth=0.75)
# plt.legend(['sax','sax_filtered'])
#
# plt.figure()
# plt.plot(t,say,t,say_filtered,linewidth=0.75)
# plt.legend(['say','say_filtered'])
...
for i in range(1,n):
    ...
    phi_error[i] = phi_kalman_r[i] - say_ref[i];
    ...
    theta_error[i] = theta_kalman_r[i] - sax_ref[i];
